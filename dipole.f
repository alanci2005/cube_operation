...
c make the sign of volA to be positive: ignore the "negative electrons"...
c this is to make the results of dipole consistent with qchem output...CPH
	volA = abs(volA)
...
C  each transition density.
      ...
      write (6,*) 'Monomer A:'
      write (6,100) titleA
      write (6,*) ' SumA = ', sumA,' e in',NelmtsA,' elements'
      write (6,96) cena(1), cena(2), cena(3)
      write (6,97) dipoleA(1),dipoleA(2),dipoleA(3)
      write (6,*) '         |Dipole| = ', muA, ' e.Ang'
      write (6,98) dipoleAD(1),dipoleAD(2),dipoleAD(3)
      write (6,*) '         |Dipole| = ', muAD, ' Debye'
      write (6,99) (quadA(i), i=1,6)
